  ...
  useEffect(() => {
    ...
    const update = (currentTime: number) => {
      ...
      if (accumulator >= frameTime) {
        ...
        // Prevent time from getting too large (reset every ~10 minutes)
        if (elapsed > 600000) {
          elapsed = elapsed % 600000;
        }
        ...
        if (!disableRotation) {
          ...
          particles.rotation.y = Math.cos(time * 0.5) * 0.15;
          particles.rotation.z = (time * 0.01 * speed) % (Math.PI * 2);
        }
    ...
    // Only recreate when essential properties change
  }, [
    particleCount,
    particleSpread, 
    particleBaseSize,
    ...
    alphaParticles,
    cameraDistance,
    // Exclude frequently changing props like speed, moveParticlesOnHover, etc.
